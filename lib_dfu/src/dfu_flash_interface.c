...
#define FLASH_ERROR() do {} while(0) // TODO - swap for assert?
...
static unsigned char current_flash_page_data[DFU_FLASH_PAGE_SIZE_BYTES];
...
{
    unsigned char *page_data_ptr = &current_flash_page_data[current_flash_subpage_index * DFU_TRANSFER_SIZE_BYTES];
    memcpy(data, page_data_ptr, DFU_TRANSFER_SIZE_BYTES);

    current_flash_subpage_index++;

    return DFU_TRANSFER_SIZE_BYTES;
}
...
int flash_cmd_write_page_data(unsigned char *data)
{
    ...
    if (current_flash_subpage_index >= NUM_DFU_PAGES_PER_FLASH_PAGE)
    {
        return 0;
    }

    memcpy(page_data_ptr, data, DFU_TRANSFER_SIZE_BYTES);
